...
import { inventoryLockSchema } from '~~/types/schemas/market'
import { NotOwnerError, ItemNotAvailableError } from '~~/server/services/errors'
import { toInventoryItemDTO } from '~~/types/dto'
...
export default defineEventHandler(async (event) => {
  ...
  const parsed = inventoryLockSchema.safeParse(await readBody(event))
  if (!parsed.success) throw createError({ statusCode: 400, statusMessage: 'Ugyldig input' })

  try {
    const item = await inventoryService.toggleLock(user.id, id, parsed.data.locked)
